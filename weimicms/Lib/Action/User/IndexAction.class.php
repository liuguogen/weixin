...
class IndexAction extends UserAction
{
	...
	"appid" => $appid,
	"id"    => array("neq", $this->_post("id", "intval"))
	))->save(array("appid" => $appid . "_no"));
	...
	public function insert()
	{
		...
		if ($this->isAgent) {
			$needPay = 0;
			if ((($this->reg_validDays * 24 * 3600) < ($users["viptime"] - $users["createtime"] - 20)) || (30 < $this->reg_validDays)) {
				$needPay = 1;
			}
				...
				if ($this->isAgent) {
					...
					if ($this->thisAgent["wxacountprice"]) {
						if ($needPay) {
							...
							M("Agent_expenserecords")->add(array("agentid" => $this->thisAgent["id"], "amount" => -$price, "des" => $this->user["username"] . "(uid:" . $this->user["id"] . ")添加公众号" . $_POST["wxname"] . "(" . $month . "个月)", "status" => 1, "time" => time()));
						}
					}
				}
	...
	public function switchTPl()
	{
			...
			if (IS_AJAX) {
				...
				else if (M("Users")->where(array("id" => (int) session("uid")))->setField("usertplid", 0)) {
					echo 200;
				}
				else {
					echo 500;
	...
	public function add_authorizer($authorizer_info, $authorization_info)
	{
		...
		if ($data["wechat_card_num"] <= $users["wechat_card_num"]) {
			$this->error("您的VIP等级所能创建的公众号数量已经到达上线，请购买后再创建", U("User/Index/index"));
			exit();
		}
			...
			if ($this->isAgent) {
				...
				if ($this->thisAgent["wxacountprice"]) {
					if ($needPay) {
						...
						$price = $month * $pricebyMonth;
						M("Agent")->where(array("id" => $this->thisAgent["id"]))->setDec("moneybalance", $price);
						M("Agent_expenserecords")->add(array("agentid" => $this->thisAgent["id"], "amount" => -$price, "des" => $this->user["username"] . "(uid:" . $this->user["id"] . ")添加公众号" . $res["wxname"] . "(" . $month . "个月)", "status" => 1, "time" => time()));
					}
				}
			}

			$save = array("appid" => $res["appid"] . "_no", "authorizer_access_token" => "", "authorizer_refresh_token" => "", "authorizer_expires" => 0);
