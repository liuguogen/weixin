...
class BargainAction extends UserAction
{
	...
	public function order()
	{
		...
		if ($_GET['orderid'] != '') {
			$where_order['tp_id'] = $_GET['orderid'] - 10000000;
	...
	public function operate()
	{
		...
		case 'fahuo':
			...
			if ($state == 0) {
				...
				$model->sendTempMsg('OPENTM200565259', array('href' => $this->siteUrl . U('Bargain/myorder', array('token' => $this->token)), 'wecha_id' => $order['wecha_id'], 'first' => '您的订单' . $orderid . '已发货', 'keyword1' => $orderid, 'keyword2' => '无', 'keyword3' => '无', 'remark' => date('Y年m月d日H时i分s秒')));
			}
