...
class LivingCircleAction extends UserAction{
	...
	public function doupdate(){
		$where_livingcircle['token'] = $this->token;
		$find_livingcircle = $this->m_livingcircle->where($where_livingcircle)->find();
		$this->handleKeyword($find_livingcircle['weimicms_id'],'LivingCircle',$_POST['keyword'],0,0);
	...
	public function typeupdate(){
		$where_type['token'] = $this->token;
		$where_type['weimicms_id'] = $_GET['typeid'];
		$find_type = $this->m_type->where($where_type)->find();
		if(empty($find_type)){
	...
	public function dotypeupdate(){
		...
		$where_type_name['weimicms_id'] = array("neq",(int)($_POST['typeid']));
		...
		if($find_type_name != null){
			...
			$where_type['token'] = $this->token;
			$where_type['weimicms_id'] = (int)($_POST['typeid']);
	...
	public function zitype(){
		$where_type['token'] = $this->token;
		$where_type['weimicms_id'] = (int)($_GET['typeid']);
		$find_type = $this->m_type->where($where_type)->find();
		if(empty($find_type)){
			$this->error("没有这个分类",U("LivingCircle/type",array("token"=>$this->token)));
			exit;
	...
	public function dozitypeadd(){
		...
		if($find_type != null){
			...
			$id_type = $this->m_type->add($add_type);
			if($id_type > 0){
				$this->success("添加分类【".$this->m_type->where(array("weimicms_id"=>$_POST['typeid']))->getField("name")."】的子分类【".$_POST['name']."】成功",U("LivingCircle/zitype",array("token"=>$this->token,"typeid"=>(int)($_POST['typeid']))));
			}
		}
	}
	//子分类修改
	public function zitypeupdate(){
		...
		$this->assign("type",$find_type);
		$where_type_zi['weimicms_id'] = (int)($_GET['zitypeid']);
		$where_type_zi['token'] = $this->token;
	...
	public function dozitypeupdate(){
		$where_type['name'] = $_POST['name'];
		$where_type['token'] = $this->token;	
		$where_type['weimicms_id'] =array("neq",(int)($_POST['zitypeid']));
		$find_type = $this->m_type->where($where_type)->find();
		if($find_type != null){
			$this->error("分类名称重复");
			exit;
			...
			$where_type_zi['token'] = $this->token;
			$where_type_zi['weimicms_id'] = (int)($_POST['zitypeid']);
	...
	public function sellcircleupdate(){
		$where_sellcircle['token'] = $this->token;
		$where_sellcircle['weimicms_id'] = (int)($_GET['sellcircleid']);
		$find_sellcircle = $this->m_sellcircle->where($where_sellcircle)->find();
		if(empty($find_sellcircle)){
	...
	public function dosellcircleupdate(){
		...
		$where_sellcircle_name['weimicms_id'] = array("neq",(int)($_POST['sellcircleid']));
		...
		$where_sellcircle['token'] = $this->token;
		$where_sellcircle['weimicms_id'] = (int)($_POST['sellcircleid']);
	...
	public function zisellcircleupdate(){
		...
		$where_sellcircle_zi['token'] = $this->token;
		$where_sellcircle_zi['weimicms_id'] = (int)($_GET['zisellcircleid']);
	...
	public function dozisellcircleupdate(){
		$where_sellcircle['token'] = $this->token;
		$where_sellcircle['name'] = $_POST['name'];
		$where_sellcircle['weimicms_id'] = array("neq",(int)($_POST['zisellcircleid']));
		$find_sellcircle = $this->m_sellcircle->where($where_sellcircle)->find();
		if(!empty($find_sellcircle)){
			$this->error("商圈名称重复");
			exit;
		...
		$where_sellcircle_zi['token'] = $this->token;
		$where_sellcircle_zi['weimicms_id'] = (int)($_POST['zisellcircleid']);
	...
	public function sellerupdate(){
		$where_seller['token'] = $this->token;
		$where_seller['weimicms_id'] = (int)($_GET['sellerid']);
	...
	public function dosellerupdate(){
		...
		$where_seller['token'] = $this->token;
		$where_seller['weimicms_id'] = (int)($_POST['sellerid']);
	...
	public function operate(){
		switch($_GET['type']){
			...
			case 'zitypedel':
				...
				$update_seller = $this->m_seller->where($where_seller)->save($save_seller);
				$where_type['token'] = $this->token;
				$where_type['weimicms_id'] = (int)($_GET['zitypeid']);
				$del_type = $this->m_type->where($where_type)->delete();
			...
			case 'zisellcircledel':
				...
				$update_seller = $this->m_seller->where($where_seller)->save($save_seller);
				$where_sellcircle['token'] = $this->token;
				$where_sellcircle['weimicms_id'] = (int)($_GET['zisellcircleid']);
				$del_sellcircle = $this->m_sellcircle->where($where_sellcircle)->delete();
	...
	public function ajax(){
		switch($_POST['type']){
			...
			case 'typedisplay':
				...
				$where_type['weimicms_id'] = (int)($_POST['id']);
			...
			case 'sellcircledisplay':
				...
				$where_sellcircle['weimicms_id'] = (int)($_POST['id']);
			...
			case 'sellerrecommend':
				...
				$where_seller['weimicms_id'] = (int)($_POST['id']);
			...
			case 'zitype':
				...
				foreach($zitype_list as $vo){
					$list.='<option value=\''.$vo['weimicms_id'].'\'>'.$vo['name'].'</option>';
				}
				$data['list'] = $list;
				if($zitype_list == ''){
					$data['error'] = 0;
...
***********************************************************************************************************商家登录***********************************************************************************************************
	*/
	...
	public function domysellergoodsadd(){
		...
		$where_seller['token'] = $this->token;
		$where_seller['cid'] = $cid;
		$sellerid = $this->m_seller->where($where_seller)->getField("weimicms_id");
		$where_mysellergoods['token'] = $this->token();
		$where_mysellergoods['cid'] = $cid;
	...
	public function mysellergoodsupdate(){
		...
		$where_mysellergoods['token'] = $this->token;
		$where_mysellergoods['cid'] = $cid;
		$where_mysellergoods['weimicms_id'] = (int)($_GET['goodsid']);
	...
	public function domysellergoodsupdate(){
		...
		$where_mysellergoods['token'] = $this->token;
		$where_mysellergoods['cid'] = $cid;
		$where_mysellergoods['weimicms_id'] = (int)($_POST['goodsid']);
		$find_goods = $this->m_mysellergoods->where($where_mysellergoods)->find();
	...
	public function sellerajax(){
		...
		switch($_POST['type']){
			...
			case 'goodsdisplay':
				...
				$where_mysellergoods['weimicms_id'] = (int)($_POST['id']);
				$where_mysellergoods['cid'] = $cid;
	...
	public function selleroperate(){
		...
		switch($_GET['type']){
			...
			case 'commentdel':
				...
				$where_comment['sellerid'] = $find_seller['weimicms_id'];
				$where_comment['weimicms_id'] = (int)($_GET['commentid']);
	...
	public function mysellerorder(){
		...
		if($_GET['name'] != ""){
			$where_order['weimicms_id'] = (int)($_GET['name']) - 10000000;
			$where_page['name'] = $_GET['name'];
		}
		import('ORG.Util.Page');
		...
		foreach($order_list as $vo){
			...
			$where_cart['orderid'] = $vo['weimicms_id'];
			$cart_list[$vo['weimicms_id']] = $this->m_mysellercart->where($where_cart)->select();
			foreach($cart_list[$vo['weimicms_id']] as $k=>$v){
				$where_goods['token'] = $this->token;
				$where_goods['weimicms_id'] = $v['goodsid'];
				$cart_list[$vo['weimicms_id']][$k]['name'] = $this->m_mysellergoods->where($where_goods)->getField('name');
				$cart_list[$vo['weimicms_id']][$k]['price'] = $this->m_mysellergoods->where($where_goods)->getField('price');
			}
		}
	...
	public function mysellerorderstup(){
		if($_GET['type'] = 'fahuo'){
			$where_order['token'] = $this->token;
			$where_order['weimicms_id'] = (int)($_GET['orderid']);
