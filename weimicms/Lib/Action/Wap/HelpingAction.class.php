	...
	public function index()
	{
		...
		if ($this->fans) {
			...
			if (!empty($this->wecha_id) && empty($us)) {
				$data = array("pid" => $this->helping["id"], "wecha_id" => $this->wecha_id, "token" => $this->token, "add_time" => time(), "help_count" => 1, "share_key" => $this->getKey());
		...
		if (($this->helping["is_attention"] == 2) && !$this->isSubscribe()) {
			...
			if ((($this->helping["is_reg"] == 1) && empty($this->fans)) || (($this->helping["is_reg"] == 1) && empty($this->fans["tel"]))) {
				$this->memberNotice();
			}
		}
