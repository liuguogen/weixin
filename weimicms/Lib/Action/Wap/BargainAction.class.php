	...
	public function operate()
	{
		...
		case 'firstdao':
			...
			if ($order == '') {
				...
				if ($bargain['qdao'] != '') {
					$kan = floor($bargain['qprice'] / $bargain['qdao']); //前N刀价格除前N刀数量    300/5=60
					if ((1 < $kan) && ($kan < $bargain['qprice'])) {
						$jian = rand(1, $kan - 1); //取1-59随机数
						$kanzhi = $kan - $jian;  //
					}
					else {
						$kanzhi = $kan;
					...
					$cha = $bargain['original'] - $bargain['minimum'];   //原价-底价
		...
		case 'friendkan':
			...
			if ($kanuser == '') {
				...
				if ($order['bargain_nowprice'] == $bargain['minimum']) {  //新的价格=砍价底价
				...
				else if ($bargain['minimum'] < $order['bargain_nowprice']) {//砍价底价<新的价格
					if ($bargain['qdao'] != '') {  //如果前N刀数量不为空
						if ($bargain['qdao'] <= $count) {  //前N刀数量小于等于已砍总数
							$cha_dao = $bargain['dao'] - $bargain['qdao'];  //总刀数减去前N刀数量

							if ($cha_dao == 1) {
								$kanzhi = $order['bargain_nowprice'] - $bargain['minimum'];
							}
							else if (1 < $cha_dao) { //需要砍价刀数不止一刀时
								$cha_dao2 = $bargain['dao'] - $count; //总刀数-已砍刀数

								if ($cha_dao2 == 1) { //总刀数减已砍刀数等于1时
									$kanzhi = $order['bargain_nowprice'] - $bargain['minimum'];
								}
								else {
									$cha_price = $bargain['original'] - $bargain['minimum'] - $bargain['qprice'];//原价-底价-前N刀价格 1000-500-300
									$kan = floor($cha_price / $cha_dao); //计算的价格 除 （总刀数减去前N刀数量）的值  200/5
					...
					$add_kanuser['addtime'] = time();
					$id_kanuser = $this->m_kanuser->add($add_kanuser);
		            $this->redirect('Bargain/dao', array('token' => $this->token, 'id' => $_GET['id'], 'orderid' => $_GET['orderid'], 'kanzhi' => abs($kanzhi)));
				}
			}
			else {
