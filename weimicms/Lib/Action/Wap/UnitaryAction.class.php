...
class UnitaryAction extends WapAction{
	...
	public function dobuy(){
		...
		$update_order = $this->m_order->where(array('tp_id'=>$id_order))->save($save_order);
		if($id_order > 0){
			//$save_cart['state'] = 1;
	...
	public function payReturn(){
		...
		if(isset($_GET['nohandle'])){
			$this->redirect("Unitary/payend",array("token"=>$this->token,"orderid"=>$order['tp_id']));
		}else{
