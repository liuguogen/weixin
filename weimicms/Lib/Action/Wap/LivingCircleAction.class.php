...
class LivingCircleAction extends WapAction{
	...
	public function typelist(){
		...
		foreach($type_list as $vo){
			...
			$where_zitype['typeid'] = $vo['weimicms_id'];
			$zitype_list[$vo['weimicms_id']] = $this->m_type->where($where_zitype)->order("num asc,addtime desc")->select();
		}
		$this->assign("type_list",$type_list);
	...
	public function sellerlist(){
		...
		foreach($sellcircle_list as $vo){
			...
			$where_zisellcircle['sellcircleid'] = $vo['weimicms_id'];
			$zisellcircle_list[$vo['weimicms_id']] = $this->m_sellcircle->where($where_zisellcircle)->order("num asc,addtime desc")->select();
		}
		...
		if((int)($_GET['sellcircleid']) != 0 && (int)($_GET['zisellcircleid']) == 0){
			...
			$where_sellcircle_name['weimicms_id'] = (int)($_GET['sellcircleid']);
			...
			$where_sellcircle_name['token'] = $this->token;
			$where_sellcircle_name['weimicms_id'] = (int)($_GET['zisellcircleid']);
			$sellcircle_name = $this->m_sellcircle->where($where_sellcircle_name)->getField("name");
		}else{
		...
		if($_GET['sort'] == 1){
			...
			foreach($seller_list as $vo){
				...
				if($find_company['display'] == 1){
					$distance[$vo['weimicms_id']] = $this->getDistance_map($this->mylat,$this->mylng,$find_company['latitude'],$find_company['longitude']);
				}
			}
			asort($distance);
			...
			foreach($seller_list as $k=>$v){
				...
				if($find_company['display'] == 1){
					...
					$sellerlist[$i]['sellerid'] = $v['weimicms_id'];
					...
					$where_comment['token'] = $this->token;
					$where_comment['sellerid'] = $v['weimicms_id'];
					$commentlist = $this->m_comment->where($where_comment)->select();
					$comcount = count($commentlist);
					$comtotal = 0;
					foreach($commentlist as $com){
	...
	public function dobuy(){
		...
		foreach($mysellercart as $vo){
			$where_mysellergoods['token'] = $this->token;
			$where_mysellergoods['weimicms_id'] = $vo['goodsid'];
		...
		$update_order = $this->m_mysellerorder->where(array('weimicms_id'=>$id_order))->save($save_order);
	...
	public function favorite(){
		...
		foreach($favorite as $vo){
			$where_seller['token'] = $this->token;
			$where_seller['weimicms_id'] = $vo['sellerid'];
			...
			if($find_company['display'] == 1){
				...
				$sellerlist[$i]['sellerid'] = $find_seller['weimicms_id'];
	...
	public function myorder(){
		if($_GET['type'] == 0){
			...
			foreach($order_list as $vo){
				$where_mysellercart['token'] = $this->token;
				$where_mysellercart['wecha_id'] = $this->wecha_id;
				$where_mysellercart['orderid'] = $vo['weimicms_id'];
				$cart_list[$vo['weimicms_id']] = $this->m_mysellercart->where($where_mysellercart)->select();
				foreach($cart_list[$vo['weimicms_id']] as $k=>$v){
					$where_mysellergoods['token'] = $this->token;
					$where_mysellergoods['sellerid'] = $v['sellerid'];
					$where_mysellergoods['weimicms_id'] = $v['goodsid'];
					$cart_list[$vo['weimicms_id']][$k]['name'] = $this->m_mysellergoods->where($where_mysellergoods)->getField("name");
					$cart_list[$vo['weimicms_id']][$k]['price'] = $this->m_mysellergoods->where($where_mysellergoods)->getField("price");
				}
			}
		...
		switch($_POST['type']){
			case 'indextjsjlist':
				...
				foreach($seller_list as $vo){
					...
					$find_company = $this->m_company->where($where_company)->find();
					if($find_company['display'] == 1){
						$distance[$vo['weimicms_id']] = $this->getDistance_map($_POST['lat'],$_POST['lng'],$find_company['latitude'],$find_company['longitude']);
					}
				}
				asort($distance);
				$i = 0;
