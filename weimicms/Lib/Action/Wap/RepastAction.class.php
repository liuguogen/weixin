	...
	"isuse"       => array("neq", 2)
		...
		if ((0 < $shopid) && ($shopid == $this->_cid)) {
			...
			if ($orid) {
				...
				if ($DishC["offtable"] == 0) {
					Sms::sendSms($this->token, "顾客$youname预定一个餐位：$tablestr，订单号：$orderid，请您注意查看并处理", $company["mp"]);
				}
				else {
					Sms::sendSms($this->token, "顾客$youname预定了一份餐，订单号：$orderid，请您注意查看并处理", $company["mp"]);
				}
	...
	public function saveOrderAndToPay()
	{
		...
		if (is_array($tmpOrderdata)) {
			...
			if ($orid) {
				...
				$company = $this->getCompany($this->_cid);
				$msgstr = "顾客$youname他刚刚点了一份餐，订单号：$orderid，请您注意查看并处理";

				if ($isjiacai) {
					$msgstr = "顾客$youname他刚刚在订单号为 $orderid 里加了菜，请您注意查看并处理";
				}
