	...
	public function index($orderid,$paytype,$third_id){
		...
		if($order){
			if($order['paid']!=1){
				...
				$where_cart['order_id'] = $order['tp_id'];
				$save_cart['state'] = 0;
			...
			foreach($cart_list as $vo){
				...
				for($i=0;$i<$vo['count'];$i++){
					...
					$add_lucknum['order_id'] = $order['tp_id'];
				...
				if($pay_count == $find_unitary['price']){
					...
					require_once './weimicms/Lib/ORG/TemplateNews.class.php';
			...
			$where_cart_count['order_id'] = $order['tp_id'];
			...
			$where_lucknum_count['order_id'] = $order['tp_id'];
