	...
	public function index($token=""){
		...
		if($service_wxuser != '' && $service_wxuser['app_id'] != ''){
			if($service_wxuser['domain'] != $domain || $service_wxuser['wxappid'] != $wxuser['appid'] || $service_wxuser['wxappsecret'] != $wxuser['appsecret']){
				...
				$data['activity_url'] = $domain."/index.php?g=Wap&m=ServiceReturn&a=activity&token=".$this->token;
				$data['msg_tip_url'] = $domain."/index.php?g=Wap&m=ServiceReturn&a=msgtip&token=".$this->token;
				$data['my_url'] = $domain."/index.php?g=Wap&m=ServiceReturn&a=my&token=".$this->token;
